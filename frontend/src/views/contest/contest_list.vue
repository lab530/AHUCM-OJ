      ...
      <div>
        ...
        >
          ...
          <template #cell(比赛名称)="data">
            <div
              class="col-name"
              @click="navigateToContest(data.item.ID)"
            >{{ data.item.title }}</div>
          </template>
...
export default {
  ...
  async created() {
    try {
        const response = await this.fetchContestList();
