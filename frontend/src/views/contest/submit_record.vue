            ...
            <div class="overflow-auto">
              ...
              >
                ...
                <template #cell(用户)="data">
                  <div
                    class="col-user"
                    @click="navigateToUser(data.item.user.ID)"
                  >{{ data.item.user.user_name }}</div>
                </template>

                <template #cell(题目编号)="data">
                  <div
                    class="col-pnum"
                    @click="navigateToProblem(data.item.problem_id)"
                  >{{ letterProblemsMap[data.item.problem_id] }}</div>
                </template>
...
export default {
  ...
  async created() {
        ...
        try {
            const submitResponse = await this.GetContestSubmit();
            this.items = submitResponse.data.data.data;
        } catch (error) {
            console.log(error);
            this.$bvToast.toast(error.response ? error.response.data.msg : '未知错误', {
                title: '数据验证错误',
